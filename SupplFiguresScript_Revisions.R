...
ggplot(discretised_plot_list) +
  geom_bar(position="dodge", stat="identity",  aes(fill = Species, alpha = Model, color = Species, x=categories, y=fraction))  +
...
ggplot(discretised_plot_list, aes(fill = Species, alpha = Model, color = Species, x=categories, y=fraction)) +
  scale_fill_manual(values = colourlist) +
  scale_color_manual(values = colourlist) +
...
ggplot(discretised_plot_list_merge, aes(fill = Species, color = Species, alpha = Mutations, x=categories, y=fraction)) +
  scale_fill_manual(values = colourlist) +
...
################################################ Suppl. Fig X: The effect of bounds of supLimit on alpha ################################################################

### to change supLimit, change integer (currently calculate 0, 1, 5 and 10)
### to be included in our estimates of DFE_alpha. This step therefore now takes quite a long time to run, 
### so have patience.

boot_statistics = data.frame()

species_list = c('Fagus_sylvatica', 'Picea_abies', 'Pinus_pinaster', 'Populus_nigra', 'Quercus_petraea', 'Betula_pendula')
model_average_statistics = data.frame()
discretised_plot_list = data.frame()
discretised_plot_list_model_avg = data.frame()
samplenumber = 40
...
for (species in species_list){
  ...
  for (val in 1:100){
    ...
    overall_boot_alpha <- sapply(overall_boot, function(e) c("supLimit = 0 " = estimateAlpha(e), "supLimit = 1 " = estimateAlpha(e, supLimit = 1), "supLimit = 5 " = estimateAlpha(e, supLimit = 5), "supLimit = 10 " = estimateAlpha(e, supLimit = 10)))
    print(overall_boot_alpha)
    
    overall_boot_alpha_model_avg =  apply(overall_boot_alpha, 1, function(x) 
      sum(sapply(1:length(overall_boot), function(i) aic_boot[i, "weight"] * x[i])) )
    
    alpha_boot_0 <- overall_boot_alpha_model_avg[1]
    alpha_boot_1 <- overall_boot_alpha_model_avg[2]
    alpha_boot_5 <- overall_boot_alpha_model_avg[3]
    alpha_boot_10 <- overall_boot_alpha_model_avg[4]

    alpha_0<- c(alpha_0, alpha_boot_0)
    alpha_1<- c(alpha_1, alpha_boot_1)
    alpha_5<- c(alpha_5, alpha_boot_5)
    alpha_10<- c(alpha_10, alpha_boot_10)
    
  }
  
  ###these steps occur per species
  statistics = data.frame(species,unlist(alpha_0), unlist(alpha_1), unlist(alpha_5), unlist(alpha_10))
  colnames(statistics) <- c('Species', "alpha0", "alpha1", "alpha5", "alpha10")
  ...
  overall_alpha <- sapply(overall, function(e) c("supLimit = 0 " = estimateAlpha(e), "supLimit = 1 " = estimateAlpha(e, supLimit = 1), "supLimit = 5 " = estimateAlpha(e, supLimit = 5), "supLimit = 10 " = estimateAlpha(e, supLimit = 10)))
  print(overall_alpha)
  
  aic = getAICweights(overall)

  overall_alpha_model_avg =  apply(overall_alpha, 1, function(x) 
    sum(sapply(1:length(overall), function(i) aic[i, "weight"] * x[i])) )
  
  alpha_dfe_0 <- overall_alpha_model_avg[1]
  alpha_dfe_1 <- overall_alpha_model_avg[2]
  alpha_dfe_5 <- overall_alpha_model_avg[3]
  alpha_dfe_10 <- overall_alpha_model_avg[4]
  
  prime_values <- data.frame(species, plotstr, alpha_dfe_0, alpha_dfe_1, alpha_dfe_5, alpha_dfe_10)
  colnames(prime_values) <- c('Species', 'name', 'alpha0', 'alpha1', 'alpha5', 'alpha10')
  model_average_statistics <- rbind(prime_values, model_average_statistics)
...
tops <- best_boots %>% group_by(Species) %>% slice_min(alpha0, n = 98)
tops <- tops %>% group_by(Species) %>% slice_max(alpha0, n = 98)
alpha_0<- ggplot()+
  geom_point(data = best_primes, aes(factor(Species), alpha0), shape=23, size=4, colour = "white", fill = "black") + 
  geom_violin(data = tops, aes(factor(Species), alpha0, fill = Species), scale = "width") + scale_fill_manual(values = colourlist) +
  geom_point(data = best_primes, aes(factor(Species), alpha0), shape=23, size=4, colour = "white", fill = "black") + 
  theme_classic() + xlab("Species") + theme(axis.text.x = element_text(angle = 90))

tops <- best_boots %>% group_by(Species) %>% slice_min(alpha1, n = 98)
tops <- tops %>% group_by(Species) %>% slice_max(alpha1, n = 98)
alpha_1<- ggplot()+
  geom_point(data = best_primes, aes(factor(Species), alpha1), shape=23, size=4, colour = "white", fill = "black") + 
  geom_violin(data = tops, aes(factor(Species), alpha1, fill = Species), scale = "width") + scale_fill_manual(values = colourlist) +
  geom_point(data = best_primes, aes(factor(Species), alpha1), shape=23, size=4, colour = "white", fill = "black") + 
  theme_classic() + xlab("Species") + theme(axis.text.x = element_text(angle = 90))

tops <- best_boots %>% group_by(Species) %>% slice_min(alpha5, n = 98)
tops <- tops %>% group_by(Species) %>% slice_max(alpha5, n = 98)
alpha_5<- ggplot()+
  geom_point(data = best_primes, aes(factor(Species), alpha5), shape=23, size=4, colour = "white", fill = "black") + 
  geom_violin(data = tops, aes(factor(Species), alpha5, fill = Species), scale = "width") + scale_fill_manual(values = colourlist) +
  geom_point(data = best_primes, aes(factor(Species), alpha5), shape=23, size=4, colour = "white", fill = "black") + 
  theme_classic() + xlab("Species") + theme(axis.text.x = element_text(angle = 90))

tops <- best_boots %>% group_by(Species) %>% slice_min(alpha10, n = 98)
tops <- tops %>% group_by(Species) %>% slice_max(alpha10, n = 98)
alpha_10<- ggplot()+
  geom_point(data = best_primes, aes(factor(Species), alpha10), shape=23, size=4, colour = "white", fill = "black") + 
  geom_violin(data = tops, aes(factor(Species), alpha10, fill = Species), scale = "width") + scale_fill_manual(values = colourlist) +
  geom_point(data = best_primes, aes(factor(Species), alpha10), shape=23, size=4, colour = "white", fill = "black") + 
  theme_classic() + xlab("Species") + theme(axis.text.x = element_text(angle = 90))

legend <- get_legend(pi_ratio_plot)

alpha0 <- alpha_0 + coord_cartesian(ylim = c(0,1)) + theme(legend.position="none")+ scale_x_discrete(labels=c("Fs", "Qp", "Bp", "Pn", "Pa", "Pp")) + labs(y= expression(paste(alpha['DFE'], ' 0')) ) + theme(axis.title.y=element_text(face="italic")) 
alpha1 <- alpha_1 + coord_cartesian(ylim = c(0,1))+ theme(legend.position="none")+ scale_x_discrete(labels=c("Fs", "Qp", "Bp", "Pn", "Pa", "Pp")) + labs(y= expression(paste(alpha['DFE'], ' 1')) ) + theme(axis.title.y=element_text(face="italic")) 
alpha5 <- alpha_5 + coord_cartesian(ylim = c(0,1))+ theme(legend.position="none")+ scale_x_discrete(labels=c("Fs", "Qp", "Bp", "Pn", "Pa", "Pp")) + labs(y= expression(paste(alpha['DFE'], ' 5')) ) + theme(axis.title.y=element_text(face="italic")) 
alpha10 <- alpha_10 + coord_cartesian(ylim = c(0,1))+ theme(legend.position="none")+ scale_x_discrete(labels=c("Fs", "Qp", "Bp", "Pn", "Pa", "Pp")) + labs(y= expression(paste(alpha['DFE'], ' 10')) ) + theme(axis.title.y=element_text(face="italic")) 


grid.arrange(
             ###plot_grid is from cowplot, and allows aligning
             plot_grid(alpha0, alpha1, alpha5, alpha10, ncol = 2, align = "hv", labels = c("A", "B", "C", "D"), label_x = 0.06, label_y = 0.93, label_fontface = "plain"),
             legend,
             layout_matrix = rbind(c(1,1,1,1,2),
                                   c(1,1,1,1,2)))
