...
pi_ratio_plot <- ggplot(data = pi_lat, aes(x = Latitude , y = pi0fold_to_pi4fold, fill = Species))+
  ...
  theme_classic() + xlab('Latitude (°N)') + ylab(expression(paste(pi[0]," /", pi[4]))) +
...
Fst_plot <- ggplot(data = pi_lat, aes(x = Latitude , y = Fst, fill = Species))+
  ...
  scale_fill_manual(values = colourlist) +
  theme(legend.text = element_text(face = c("italic"))) +
  theme_classic() + xlab('Latitude (°N)') + ylab(expression(paste("F"["ST"]))) + coord_cartesian(ylim = c(0, 0.5))
...
### alpha_model_avg[2] = limit of minimum 1 (supLimit) for strength of selection on beneficial mutations.
...
for (species in species_list){
  ...
  for (val in 1:100){
    ...
    alpha_boot <- overall_boot_alpha_model_avg[3]
...
#  disc_dfe_model_avg = sum(sapply(1:length(overall), function(i) aic[i, "weight"] * disc_dfe[1,][i]))
  ...
  discretised_plot_list_model_avg = rbind(discretised_plot_list_model_avg, dfe)
  alpha_dfe <- overall_alpha_model_avg[3]
  print(alpha_dfe)
...
compareModels("/Users/jennyjames/Dropbox/LASCOUX/GenTree/assign_ancestral/PolyDFE_GenTree_Clean/BpenQpet_indep.out", "/Users/jennyjames/Dropbox/LASCOUX/GenTree/assign_ancestral/PolyDFE_GenTree_Clean/BpenQpet_share.out")
compareModels("/Users/jennyjames/Dropbox/LASCOUX/GenTree/assign_ancestral/PolyDFE_GenTree_Clean/BpenQpet_indep.out", "/Users/jennyjames/Dropbox/LASCOUX/GenTree/assign_ancestral/PolyDFE_GenTree_Clean/BpenQpet_shareb.out")
